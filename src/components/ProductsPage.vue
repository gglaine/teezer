<template>
    ...
    <button @click="toggleSortOrder" class="w-full border-1 mt-2 mb-2 text-right border-white text-white p-2 rounded bg-transparent">
        Sort Price {{ sortOrder === 'ASC' ? '🔽' : '🔼' }}
    </button>
      <div class="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4">
        <div v-for="(product, index) in products" :key="product.id" @click="openLightbox(product)" class="group cursor-pointer mb-4 break-inside-avoid-column relative max-w-sm rounded overflow-hidden shadow-lg">
            ...
            aspect-ratio="1"
            />
            <button @click.stop="toggleCaption(product.id, $event)" class="absolute bottom-4 right-4 bg-slate-700 z-50 w-10 text-white p-2 rounded-full">+</button>
    ...
    methods: {
        ...
        async fetchProducts() {
            ...
            this.products = productsData.map((product, index) => {
                ...
                figcaption: product.figcaption.split('    Completed')[0].trim(),
                price: priceMatch ? parseFloat(priceMatch[0].replace(/[$,]/g, '')) : 0 // Convert price to number
                };
            });
